...
`, name)), 0644)
	...
	// Generate SDK files
	sdkFiles := generateDriverSDK(driverPath, author, name)
...
func generateDriverSDK(driverPath, author, name string) []string {
	...
	rustSDKPath := filepath.Join(driverPath, "sdk", "rust")
	rustSrcPath := filepath.Join(rustSDKPath, "src")
	goSDKPath := filepath.Join(driverPath, "sdk", "go")
...
//! [dependencies]
//! wazeos-app = { path = "../../core/sdk/rust/app" }
//! wazeos-%s-%s = { path = "../../drivers/%s/sdk/rust" }
//! ` + "```" + `
//!
...
` + "```toml" + `
[dependencies]
wazeos-%s-%s = { path = "~/.wazeos/drivers/%s/sdk/rust" }
` + "```" + `
...
//	replace github.com/wazeos/wazeos/drivers/%s/sdk/go => ../../drivers/%s/sdk/go
//
// # Complete Example
//
...
` + "```go" + `
require github.com/wazeos/wazeos/drivers/%s/sdk/go v0.1.0
replace github.com/wazeos/wazeos/drivers/%s/sdk/go => ~/.wazeos/drivers/%s/sdk/go
` + "```" + `
...
const goDriverTemplate = `package {{.Package}}

// {{.Name}} - WazeOS I/O Driver
//
// This driver extends the WazeOS kernel with {{.Class}} capabilities.
//
// Quick Start:
//   Build:    go build -buildmode=plugin -o build/{{.Name}}.so .
//             (or: wazeos driver build {{.Name}})
//   Test:     go test -v ./...
//             (or: wazeos driver test {{.Name}})
//   Install:  wazeos driver install {{.Name}}
//
// Development:
//   - Implement the Driver interface methods below
//   - Use Call() to handle URI-based requests
//   - Return proper errors with descriptive messages
//   - Test with integration tests before deployment
...
// Driver implements {{.Class}} operations
type Driver struct {
	// TODO: Add driver-specific fields here (config, connections, etc.)
}

func init() {
	// Self-register with IO Bus
	iobus.Register(iobus.DriverSpec{
		Name:    "{{.Name}}",
		Version: "1.0.0",
		Class:   iobus.{{.Class}}Driver,

		URIPattern: "{{.Name}}://**",

		Capabilities: []iobus.Capability{
			iobus.CapCall,
...
func (d *Driver) Close() error {
	// TODO: Cleanup resources when driver shuts down
	//
	// Examples:
...
func (d *Driver) Call(ctx iobus.Context, req iobus.Request) (iobus.Response, error) {
	// TODO: Implement driver logic here
	//
	// Parse the request URI and handle the operation:
	...
	//
	// Return responses:
	//   Success: return iobus.NewResponse(200, responseBody), nil
	//   Error:   return iobus.NewResponse(400, nil), fmt.Errorf("error message")
	//
	// Call other drivers if needed:
	//   resp, err := ctx.Call("file:///path/to/file", iobus.Request{...})

	return iobus.NewResponse(200, []byte("not implemented")), nil
}
`
...
func runDriverBuild(cmd *cobra.Command, args []string) {
	...
	logInfo("Building Go driver: %s", name)

	// Build driver as plugin
	buildCmd := fmt.Sprintf("cd %s && go build -buildmode=plugin -o build/%s.so .", driverPath, name)
	if driverRelease {
		buildCmd = fmt.Sprintf("cd %s && go build -buildmode=plugin -ldflags='-s -w' -o build/%s.so .", driverPath, name)
	}
...
func runDriverInstall(cmd *cobra.Command, args []string) {
	...
	if jsonOutput {
		...
		logInfo("SDK: %s/sdk/", installDir)
		logInfo("")
		logInfo("Apps can now use this driver's SDK:")
		logInfo("  Rust: [dependencies]")
		logInfo("        wazeos-%s-%s = { path = \"%s/sdk/rust\" }", author, name, installDir)
		logInfo("  Go:   import \"%s_%ssdk\" from \"%s/sdk/go\"", toSnakeCase(author), toSnakeCase(name), installDir)
	}
}
